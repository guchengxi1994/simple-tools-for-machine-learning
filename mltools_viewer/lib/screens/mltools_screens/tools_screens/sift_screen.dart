...
class SiftScreen extends BaseToolsScreen {
  SiftScreen({Key? key})
      : super(key: key, barTitle: "图像相似度对比(基于SIFT)", onShowCodeClicked: () {});

  @override
  BasePageState<BaseToolsScreen> getState() => _SiftScreenState();
}

class _SiftScreenState extends BasePageState<BaseToolsScreen> {
            ...
            children: [
              ...
              ElevatedButton(
                  onPressed: () async {
                    ...
                    Response? res = await dioUtils.post(mltoolsApis['sift'],
                        data: req.toJson());
                    // print(res);
                    if (res != null) {
                      SiftResp resp = SiftResp.fromJson(res.data);
                      if (resp.code != 200) {
                        ToastUtils(context).showToast(resp.message ?? "错误");
                      } else {
                        ...
                        if (imageKey1.currentContext!.size != null) {
                          imageWidth =
                              imageKey1.currentContext!.size!.width.ceil();
                        } else {
                          ToastUtils(context)
...
class SiftPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final i in matches) {
      ...
      List left = i[0];
      List right = i[1];
      canvas.drawLine(Offset(left[1] * 1.0, left[0] * 1.0),
          Offset(right[1] * 1.0 + image1Width, right[0] * 1.0), line);
      count += 1;
    }
  }
