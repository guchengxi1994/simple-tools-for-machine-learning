self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={yc:function yc(d,e){this.a=d
this.b=e},fH:function fH(d,e,f){this.a=d
this.b=e
this.$ti=f},
ali(d,e,f){var x=0,w=A.S(y.f),v,u,t,s
var $async$ali=A.T(function(g,h){if(g===1)return A.P(h,w)
while(true)switch(x){case 0:u=(self.URL||self.webkitURL).createObjectURL(A.apy([C.F.geb().bn(e)]))
t=document
s=t.createElement("a")
y.d.a(s)
s.href=u
v=s.style
v.display="none"
s.download=f
v=t.body
if(v!=null)v.appendChild(s)
s.click()
t=t.body
if(t!=null)A.asM(t,s);(self.URL||self.webkitURL).revokeObjectURL(u)
t=E.u2(d)
t.m4("\u6587\u4ef6\u4e0b\u8f7d\u5b8c\u6210")
return A.Q(null,w)}})
return A.R($async$ali,w)},
atF(d,e){d=d+e&536870911
d=d+((d&524287)<<10)&536870911
return d^d>>>6}},A,J,C,D,E
B=a.updateHolder(c[23],B)
A=c[0]
J=c[1]
C=c[2]
D=c[58]
E=c[31]
B.yc.prototype={
j(d){return"MltoolType."+this.b}}
B.fH.prototype={
j(d){return"["+A.e(this.a)+", "+A.e(this.b)+"]"},
k(d,e){if(e==null)return!1
return e instanceof B.fH&&J.f(e.a,this.a)&&J.f(e.b,this.b)},
gA(d){var x,w=J.t(this.a),v=J.t(this.b)
v=B.atF(B.atF(0,C.f.gA(w)),C.f.gA(v))
x=v+((v&67108863)<<3)&536870911
x^=x>>>11
return x+((x&16383)<<15)&536870911}}
var z=a.updateTypes([]);(function inheritance(){var x=a.inherit
x(B.yc,A.dV)
x(B.fH,A.z)})()
A.bo(b.typeUniverse,JSON.parse('{"yc":{"N":[]}}'))
var y={d:A.B("qn"),f:A.B("~")};(function constants(){D.nM=new A.bm(57965,!1)
D.ib=new B.yc(1,"forImage")})()}
$__dart_deferred_initializers__["wOorPelk4P/2frtn2OvFdS8qzyo="] = $__dart_deferred_initializers__.current
